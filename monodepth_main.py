...
#    args.checkpoint_path="C:/Temp/mono_depth/checkpoints/vgg/model-16000.index"
    ...
    print("We are here ",args.checkpoint_path,"*")
    ...
    for step in range(num_test_samples):
        if 'semantic' in args.task and 'depth' in args.task:
            ...
            #cv2.imshow("result", semantics[step])
            print("depth+semantic")
        elif 'depth' in args.task:
            disp = sess.run(model.disp_left_est[0])
            disparities[step] = disp[0].squeeze()
            disparities_pp[step] = post_process_disparity(disp.squeeze())
            #cv2.imshow("result", disparities_pp[step])
            print("depth")
...
def recognize(params):
    ...
    model = MonodepthModel(params, args.mode, args.task, left, left, semantic, valid)
    ...
    for count in range (loop_size):
        for step in range(100):
            step_=count*100+step
            if 'semantic' in args.task and 'depth' in args.task:
                disp, sem = sess.run([model.disp_left_est[0], tf.argmax(model.sem_est[0], -1)])
                disparities[step_] = disp[0].squeeze()
                disparities_pp[step_] = post_process_disparity(disp.squeeze())
                semantics[step_] = sem[0].squeeze()
            elif 'depth' in args.task:
                disp = sess.run(model.disp_left_est[0])
                disparities[step_] = disp[0].squeeze()
                disparities_pp[step_] = post_process_disparity(disp.squeeze())
        ...
        progress=int(count/loop_size*100)
